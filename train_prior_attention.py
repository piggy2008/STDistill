...
from misc import AvgMeter, check_mkdir, CriterionDice
...
args = {
    ...
    'momentum': 0.95,
    'snapshot': '',
    'pretrain': os.path.join(ckpt_path, 'VideoSaliency_2019-12-19 10:43:54', '50000.pth'),
    # 'pretrain': '',
...
if args['dice']:
    criterion_dice = CriterionDice().cuda()
...
def main():
    net = R3Net_prior(motion=args['motion'], se_layer=args['se_layer'],
                      ...
                      isTriplet=args['isTriplet'], basic_model=args['basic_model'], sta=args['sta'], naive_fuse=args['naive']).cuda().train()
...
def train(net, optimizer):
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            if args['isTriplet']:
                outputs0, outputs1, outputs2, outputs3, outputs4, outputs5, outputs_triplet = net(inputs)
            else:
            ...
            if args['L2']:
                loss0 = loss0 + 0.1 * criterion_l2(torch.relu(outputs0) / torch.max(outputs0), labels)
                loss1 = loss1 + 0.1 * criterion_l2(torch.relu(outputs1) / torch.max(outputs1), labels.narrow(0, 1, 4))
                loss2 = loss2 + 0.1 * criterion_l2(torch.relu(outputs2) / torch.max(outputs2), labels.narrow(0, 2, 3))
                loss3 = loss3 + 0.1 * criterion_l2(torch.relu(outputs3) / torch.max(outputs3), labels.narrow(0, 3, 2))
                loss4 = loss4 + 0.1 * criterion_l2(torch.relu(outputs4) / torch.max(outputs4), labels.narrow(0, 4, 1))

            if args['dice']:
                loss0 = loss0 + 0.5 * criterion_dice(outputs0, labels)
                loss1 = loss1 + 0.5 * criterion_dice(outputs1, labels.narrow(0, 1, 5))
                loss2 = loss2 + 0.5 * criterion_dice(outputs2, labels.narrow(0, 2, 4))
                loss3 = loss3 + 0.5 * criterion_dice(outputs3, labels.narrow(0, 3, 3))
                loss4 = loss4 + 0.5 * criterion_dice(outputs4, labels.narrow(0, 4, 2))
                loss5 = loss4 + 0.5 * criterion_dice(outputs5, labels.narrow(0, 5, 1))
